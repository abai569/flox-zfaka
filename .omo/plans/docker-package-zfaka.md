...
为 ZFAKA 创建可部署的 Docker 运行时和 GitHub Actions 发布流程，镜像发布到 `ghcr.io/abai569/flox-zfaka`，标签使用 `1.4.7` 与 `latest`。
...
6. 更新源码版本为 `1.4.7`，并确保 `ADMIN_DIR` 可由环境变量控制。
...
- GHCR 工作流仅在 `release/v1.4.7` 分支推送时发布镜像。
...
```sh
rg -n "ZFAKA_VERSION|ADMIN_DIR|1\.4\.7" application/init.php conf/application.ini.template docker-compose.yml Dockerfile
```

验收：`application/init.php` 从环境变量读取 `ZFAKA_VERSION` 与 `ADMIN_DIR`，并分别使用 `1.4.7` 与 `Goadmin` 作为默认值；Compose 默认版本为 `1.4.7`。使用 `ADMIN_DIR=CustomAdmin` 启动时，entrypoint 将后台模块目录改为 `CustomAdmin`，并渲染配置中的应用目录。
...
```sh
rg -n "branches: \[main\]|tags: \['v\*\.\*\.\*'\]|platforms: linux/amd64,linux/arm64|push: true" .github/workflows/docker-publish.yml
```

验收：工作流只声明 `release/v1.4.7` push 触发器，不包含 `workflow_dispatch`，并使用 Buildx 推送固定的 `1.4.7` 与 `latest` 标签；发布后执行以下命令：

```sh
docker buildx imagetools inspect ghcr.io/abai569/flox-zfaka:1.4.7
