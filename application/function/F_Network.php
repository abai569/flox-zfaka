...
if ( ! function_exists('getIpInfo')){
	function getIpInfo($ip){
		if($ip && filter_var($ip, FILTER_VALIDATE_IP) && function_exists('curl_init')){
			$url='https://ipwho.is/'.rawurlencode($ip).'?lang=zh-CN&fields=success,country,region,city';
			$ch = curl_init();
			...
			if($result){
				...
				if(is_array($result) AND isset($result['success']) AND $result['success']){
					return array(
						'ip'=>$ip,
						'country'=>isset($result['country']) && is_string($result['country']) ? $result['country'] : '',
						'region'=>isset($result['region']) && is_string($result['region']) ? $result['region'] : '',
						'city'=>isset($result['city']) && is_string($result['city']) ? $result['city'] : ''
					);
				}
			}
		}
