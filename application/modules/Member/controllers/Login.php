			...
			if ($this->VerifyCsrfToken($csrf_token)) {
				...
				if(isEmail($email)){
					...
					if($checkUser){
						...
						$ip = getClientIP();
						$ipInfo = getIpInfo($ip);
						$region = $ipInfo ? trim(implode(' ', array_unique(array_filter(array($ipInfo['country'], $ipInfo['region'], $ipInfo['city']))))) : '';
						$m=array('userid'=>$checkUser['id'],'ip'=>$ip,'region'=>$region,'addtime'=>time());
